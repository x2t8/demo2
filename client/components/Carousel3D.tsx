...
export default function Carousel3D({ modules, onModuleChange }: Carousel3DProps) {
  ...
  const get3DTransform = (position: number) => {
    switch (position) {
      case 0: // center - no rotation
        return "rotateY(0deg)";
      case -1: // left adjacent - rotate -10°
        return "rotateY(-10deg)";
      case 1: // right adjacent - rotate +10°
        return "rotateY(10deg)";
      case -2: // left outer - rotate -20°
        return "rotateY(-20deg)";
      case 2: // right outer - rotate +20°
        return "rotateY(20deg)";
      default:
        return "rotateY(0deg)";
    }
        ...
        {visibleModules.map(({ module, originalIndex, position }) => {
          ...
          return (
            <div
              ...
              className={cn(
                "absolute top-1/2 transition-all duration-500 ease-in-out cursor-pointer",
                position === 0 && "left-1/2 -translate-x-1/2 -translate-y-1/2 translate-y-2.5 z-50 scale-100 opacity-100",
                position === -1 && "left-[30%] -translate-x-1/2 -translate-y-1/2 z-40 scale-90 opacity-80 hover:scale-95 hover:opacity-90",
                position === 1 && "right-[30%] translate-x-1/2 -translate-y-1/2 z-40 scale-90 opacity-80 hover:scale-95 hover:opacity-90",
                position === -2 && "left-[10%] -translate-x-1/2 -translate-y-1/2 z-30 scale-80 opacity-50 hover:scale-85 hover:opacity-60",
                position === 2 && "right-[10%] translate-x-1/2 -translate-y-1/2 z-30 scale-80 opacity-50 hover:scale-85 hover:opacity-60",
                Math.abs(position) > 2 && "opacity-0 pointer-events-none scale-75"
              )}
              onClick={() => !isCenter && handleModuleClick(originalIndex)}
              style={{
                transform: get3DTransform(position),
                transformStyle: "preserve-3d"
              }}
            >
              <Card
                className={cn(
                  "w-64 lg:w-72 bg-white/98 dark:bg-gray-800/98 backdrop-blur-md border transition-all duration-500",
              ...
              >
                <CardContent className={cn(
                  ...
                  {/* Badge - Very subtle with opacity 0.3 border */}
                  <div className="flex justify-center mb-6">
                    <Badge 
                      variant="outline" 
                      className={cn(
                      ...
                      "border-gray-300/30 dark:border-gray-600/30 bg-gray-200/20 dark:bg-gray-700/20"
                      )}
                    >
